{if $stats->total eq 0}
<div class="mx-1 flex-column justify-center items-center shadow rounded py-1 px-2" style="height: 200px">
    <p class="text-center">Aucun evénément à afficher</p>
</div>
{else}
                ...
                {foreach from=$events item=event key=i}
                    <tr>
                        ...
                        <td class="align-middle">
                            <span>{$event->startDateTime|date_format:"%d/%m/%Y"}</span><br>
                            <span>{$event->startDateTime|date_format:"%H:%M"}</span>
                        </td>
                        <td  class="align-middle">
                            <span>{$event->endDateTime|date_format:"%d/%m/%Y"}</span><br>
                            <span>{$event->endDateTime|date_format:"%H:%M"}</span>
                        </td>
                        <td  class="align-middle">
                            {$event->location}
                        </td>
